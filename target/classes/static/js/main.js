...
var uploadExcelForm = document.querySelector('#uploadExcelForm');
...
function uploadExcelFile(file) {
    ...
    xhr.onload = function() {
        ...
        if(xhr.status == 200) {
        	...
        	uploadExcelSuccess.innerHTML = "<p>File Uploaded Successfully.</p><p>DownloadUrl : <a href='" + response.fileDownloadUri + "' target='_blank'>" + response.fileDownloadUri + "</a></p>";
        	uploadExcelSuccess.style.display = "block";
        } else {
        	uploadExcelSuccess.style.display = "none";
        	uploadExcelError.innerHTML = (response && response.message) || "Some Error Occurred";
        }
    }
...
uploadExcelForm.addEventListener('submit', function(event){
    var files = uploadExcelInput.files;
    if(files.length === 0) {
    	uploadExcelError.innerHTML = "Please select a file";
